...
type TranslationDirection = 'toGaster'; // Only support English to Gaster

export default function GasterTranslatorTool({
  pageData,
  locale = 'en',
}: GasterTranslatorToolProps) {
  const [inputText, setInputText] = useState<string>('');
  const [outputText, setOutputText] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [direction] = useState<TranslationDirection>('toGaster'); // Fixed direction
  ...
  const handleTranslate = async () => {
    ...
    try {
      const response = await fetch('/api/gaster-translator', {
        method: 'POST',
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `gaster-translation-${Date.now()}.txt`;
        ...
        <div className="flex flex-col md:flex-row gap-2 md:gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <textarea
              ...
              placeholder="Enter English text to convert to Gaster..."
