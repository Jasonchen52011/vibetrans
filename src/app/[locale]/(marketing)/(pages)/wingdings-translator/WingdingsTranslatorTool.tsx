...
interface WingdingsTranslatorToolProps {
  pageData: any;
  locale?: string;
}

export default function WingdingsTranslatorTool({
  pageData,
  locale = 'en',
}: WingdingsTranslatorToolProps) {
  ...
  const handleAudioSelected = async (
    ...
    try {
      ...
      const response = await fetch('/api/wingdings-translator/transcribe', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleTranslate = async () => {
    ...
    try {
      const response = await fetch('/api/wingdings-translator', {
        method: 'POST',
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      smartDownload(outputText, 'wingdings-translator', {
        onSuccess: () => {
          // 可以添加成功提示
        },
        onError: (error) => {
