...
export default function MiddleEnglishTranslatorTool({ pageData, locale = 'en' }: MiddleEnglishTranslatorToolProps) {
  const [inputText, setInputText] = useState<string>('');
  const [outputText, setOutputText] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [direction, setDirection] = useState<'modern-to-middle' | 'middle-to-modern'>('modern-to-middle');
  const [dialect, setDialect] = useState<'general' | 'northern' | 'kentish' | 'midlands'>('general');
  const [period, setPeriod] = useState<'1150-1300' | '1300-1450' | '1450-1500'>('1300-1450');
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `middle-english-translator-${Date.now()}.txt`;
      ...
      <main className="w-full bg-white dark:bg-zinc-800 shadow-xl border border-gray-100 dark:border-zinc-700 rounded-lg p-4 md:p-8">
        ...
        <div className="mb-6 flex flex-wrap gap-4 items-center justify-center p-4 bg-gray-50 dark:bg-zinc-700/50 rounded-lg">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-200">Dialect:</label>
            <select
              value={dialect}
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start">
          ...
          <div className="flex-1 w-full">
            <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-100 mb-3">
              {direction === 'modern-to-middle' ? 'Modern English Text' : 'Middle English Text'}
            </h2>
            <textarea
              value={inputText}
          ...
          <div className="flex md:flex-col items-center justify-center shrink-0 self-center">
            <button
              onClick={() => setDirection(direction === 'modern-to-middle' ? 'middle-to-modern' : 'modern-to-middle')}
              className="p-2 text-gray-400 hover:text-gray-600 dark:text-gray-600 dark:hover:text-gray-400 transition-colors"
              title={direction === 'modern-to-middle' ? 'Modern → Middle' : 'Middle → Modern'}
          ...
          <div className="flex-1 w-full">
            <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-100 mb-3">
              {direction === 'modern-to-middle' ? 'Middle English Translation' : 'Modern English Translation'}
            </h2>
