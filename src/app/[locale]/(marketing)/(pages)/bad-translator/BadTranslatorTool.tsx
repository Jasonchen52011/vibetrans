...
export default function BadTranslatorTool({
  pageData,
  locale = 'en',
}: BadTranslatorToolProps) {
  ...
  const [style, setStyle] = useState<'humor' | 'absurd' | 'funny' | 'chaos'>(
    'humor'
  );
  ...
  const handleTranslate = async () => {
    ...
    try {
      const response = await fetch('/api/bad-translator', {
        method: 'POST',
        headers: {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `bad-translated-${style}-${Date.now()}.txt`;
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {pageData.tool.iterationsLabel}
            </label>
            <select
              value={iterations}
              onChange={(e) => setIterations(Number(e.target.value))}
              className="w-full px-3 py-2 pr-12 border border-gray-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-700 text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-primary focus:border-transparent appearance-none"
              style={{
                backgroundImage:
          ...
          <div>
            ...
            >
              <option value="humor">{pageData.tool.styles.humor}</option>
              <option value="absurd">{pageData.tool.styles.absurd}</option>
              <option value="funny">{pageData.tool.styles.funny}</option>
              <option value="chaos">{pageData.tool.styles.chaos}</option>
            </select>
          </div>
        </div>
