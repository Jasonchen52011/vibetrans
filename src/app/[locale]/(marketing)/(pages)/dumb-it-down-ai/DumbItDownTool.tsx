...
interface DumbItDownToolProps {
  pageData: any;
  locale?: string;
}

export default function DumbItDownTool({
  pageData,
  locale = 'en',
}: DumbItDownToolProps) {
  ...
  const handleSimplify = async () => {
    ...
    try {
      const response = await fetch('/api/dumb-it-down-ai', {
        method: 'POST',
        headers: {
    ...
    <div className="container max-w-7xl mx-auto px-4 mb-10">
      <main className="w-full bg-white dark:bg-zinc-800 shadow-xl border border-gray-100 dark:border-zinc-700 rounded-lg p-4 md:p-8">
        ...
        <div className="mt-6 flex justify-center gap-4">
          ...
          >
            {isLoading ? pageData.tool.loading : pageData.tool.simplifyButton}
            <i
              className={`${isLoading ? 'fas fa-spinner animate-spin' : 'fas fa-brain'} ms-2`}
            ></i>
          </button>
          <button
            onClick={handleReset}
