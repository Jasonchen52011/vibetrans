...
interface GenZTranslatorToolProps {
  pageData: any;
  locale?: string;
}

export default function GenZTranslatorTool({
  pageData,
  locale = 'en',
}: GenZTranslatorToolProps) {
  const [inputText, setInputText] = useState<string>('');
  const [translatedText, setTranslatedText] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [mode, setMode] = useState<'toGenZ' | 'toStandard'>('toGenZ');
  ...
  const handleTranslate = async () => {
    ...
    try {
      const response = await fetch('/api/gen-z-translator', {
        method: 'POST',
        headers: {
  ...
  const toggleMode = () => {
    ...
    setMode((prev) => (prev === 'toGenZ' ? 'toStandard' : 'toGenZ'));
    setFileName(null);
    setError(null);
  };
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `translated-${mode}-${Date.now()}.txt`;
