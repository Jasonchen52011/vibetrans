...
interface DrowTranslatorToolProps {
  pageData: any;
  locale?: string;
}

export default function DrowTranslatorTool({
  pageData,
  locale = 'en',
}: DrowTranslatorToolProps) {
  ...
  const handleAudioSelected = async (
    ...
    try {
      ...
      const response = await fetch('/api/drow-translator/transcribe', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/drow-translator', {
        method: 'POST',
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      smartDownload(outputText, 'drow-translator', {
        onSuccess: () => {
          // 可以添加成功提示
        },
        onError: (error) => {
