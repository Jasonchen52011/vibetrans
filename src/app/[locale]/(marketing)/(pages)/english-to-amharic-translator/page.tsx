...
import EnglishToAmharicTranslatorTool from './EnglishToAmharicTranslatorTool';
...
interface EnglishToAmharicTranslatorPageProps {
  params: Promise<{ locale: Locale }>;
}

// 创建异步组件来包裹翻译器内容
async function TranslatorContent({ locale }: { locale: string }) {
  const t = await getTranslations({
    locale,
    namespace: 'EnglishToAmharicTranslatorPage',
  });
  ...
  return (
    <>
      ...
      <div className="pt-0 pb-12 bg-gradient-to-b from-muted/20 to-background">
        <EnglishToAmharicTranslatorTool
...
export default async function EnglishToAmharicTranslatorPage(
  props: EnglishToAmharicTranslatorPageProps
) {
    ...
    <>
      ...
      <div className="flex flex-col">
        ...
        <Suspense fallback={<HeroSkeleton />}>
          <TranslatorContent locale={locale} />
        </Suspense>
      </div>
    </>
  );
