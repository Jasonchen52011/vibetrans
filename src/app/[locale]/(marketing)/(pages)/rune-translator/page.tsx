...
export async function generateMetadata({
  ...
  const gt = await getTranslations({ locale, namespace: 'RuneTranslatorPage' });

  return constructMetadata({
    title: `${gt('title')} | ${(t as any)('name')}`,
    description: gt('description'),
    canonicalUrl: getUrlWithLocale('/rune-translator', locale),
    image: (gt as any)('whatIs.image'),
  });
}

interface RuneTranslatorPageProps {
  params: Promise<{ locale: Locale }>;
}

export default async function RuneTranslatorPage(
  props: RuneTranslatorPageProps
) {
  const params = await props.params;
  const { locale } = params;
  const t = await getTranslations({ locale, namespace: 'RuneTranslatorPage' });

  // Structured Data for SEO
  const structuredData = buildToolStructuredData({
    name: 'VibeTrans Rune Translator',
  ...
  const howtoSection = {
    ...
    image: {
      src: (t as any)('whatIs.image'),
      alt: 'How to use Rune Translator',
    },
    items: [
      {
  ...
  // Page data consumed by the tool component
  const pageData = {
    tool: {
    ...
    <>
      ...
      <div className="flex flex-col">
        ...
        <div className="pt-0 pb-12 bg-gradient-to-b from-muted/20 to-background">
          <RuneTranslatorTool pageData={pageData} locale={locale} />
        </div>
        ...
        {/* Testimonials */}
        <TestimonialsThreeColumnSection namespace="RuneTranslatorPage.testimonials" />

        {/* FAQ */}
        <FaqSection namespace="RuneTranslatorPage.faqs" />

        {/* CTA */}
        <CallToActionSection namespace="RuneTranslatorPage.cta" />
      </div>
    </>
  );
}
