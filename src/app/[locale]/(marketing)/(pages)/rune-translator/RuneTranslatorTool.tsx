...
interface RuneTranslatorToolProps {
  pageData: any;
  locale?: string;
}

export default function RuneTranslatorTool({
  pageData,
  locale = 'en',
}: RuneTranslatorToolProps) {
  ...
  const handleTranslate = async () => {
    ...
    try {
      // TODO: 替换为你的 API 端点
      const response = await fetch('/api/rune-translator', {
        method: 'POST',
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      smartDownload(outputText, 'rune-translator', {
        onSuccess: () => {
          // 可以添加成功提示
        },
        onError: (error) => {
