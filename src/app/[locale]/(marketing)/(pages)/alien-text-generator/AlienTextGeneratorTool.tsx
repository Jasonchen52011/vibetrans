...
export default function AlienTextGeneratorTool({
  pageData,
  locale = 'en',
}: AlienTextGeneratorToolProps) {
  const [inputText, setInputText] = useState<string>('');
  const [generatedText, setGeneratedText] = useState<string>('');
  const [selectedStyle, setSelectedStyle] = useState<AlienStyle>('symbols');
  const [isGenerating, setIsGenerating] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);

  const alienStyles = getAlienStyles();
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Use native conversion (no API needed)
      const result = convertToAlienText(inputText, selectedStyle);
      ...
      <main className="w-full bg-white dark:bg-zinc-800 shadow-xl border border-gray-100 dark:border-zinc-700 rounded-lg p-4 md:p-8">
        ...
        <div className="mb-6">
          <label
            ...
            className="block text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3"
          >
            {pageData.tool.selectStyle}
          </label>
          <select
            id="style-select"
            value={selectedStyle}
        ...
        <div className="mt-6 flex justify-center gap-4">
          ...
          >
            {isGenerating
              ...
              : pageData.tool.generateButton}
            <i
              className={`${isGenerating ? 'fas fa-spinner animate-spin' : 'fas fa-magic'} ms-2`}
            ></i>
          </button>
          <button
            onClick={handleReset}
