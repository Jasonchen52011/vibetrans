...
function translateToAncientGreek(text: string): string {
  ...
  // 按长度排序，优先匹配更长的词
  const sortedKeys = Object.keys(ancientGreekTranslations).sort(
...
function translateToEnglish(text: string): string {
  ...
  // 按长度排序，优先匹配更长的词
  const sortedKeys = Object.keys(englishTranslations).sort(
    (a, b) => b.length - a.length
  );
  ...
  try {
    ...
    return NextResponse.json({
      ...
      detectedInputLanguage:
        detectedDirection === 'english-to-greek' ? 'english' : 'ancient-greek',
    });
  } catch (error) {
