...
/**
 * 顺序生成 Aramaic Translator 缺失的图片
 * 使用火山4.0和Seedream循环生成，避免并发限制
 */
...
async function main() {
  console.log('🚀 开始顺序生成 Aramaic Translator 图片');
  console.log('🔄 策略: 火山4.0 ↔️ Seedream 4.0 循环使用');
  console.log('⏱️  每张图片之间等待 5 秒\n');
  ...
  if (result.success) {
    console.log('\n✅ 图片生成完成');
    console.log(`📊 成功: ${result.successfulImages}/${result.totalImages}`);
    process.exit(0);
  } else {
    console.error('\n❌ 图片生成失败');
    console.error(`📊 成功: ${result.successfulImages}/${result.totalImages}`);
    process.exit(1);
  }
}
