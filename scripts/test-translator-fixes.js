...
function checkMessageFile(translatorName) {
  const filePath = path.join(
    __dirname,
    `../messages/pages/${translatorName}/en.json`
  );
...
function getSpecificLabelFields(translatorName) {
  const labelMap = {
    ...
    'high-valyrian-translator': [
      'highValyrianLabel',
      'highValyrianPlaceholder',
    ],
    'ivr-translator': ['ivrLabel', 'ivrPlaceholder'],
    'middle-english-translator': [
      'middleEnglishLabel',
      'middleEnglishPlaceholder',
    ],
...
function generateReport() {
  ...
  results.sort((a, b) => {
    ...
    if (!a.isPerfect && b.isPerfect) return -1;
    return (
      b.missingCount + b.hardcodedCount - (a.missingCount + a.hardcodedCount)
    );
  });
  ...
  results.forEach((result) => {
    ...
    if (result.missingCount > 0) {
      console.log(
        `   缺失字段 (${result.missingCount}): ${result.messageCheck.missing.slice(0, 3).join(', ')}${result.messageCheck.missing.length > 3 ? '...' : ''}`
      );
    }

    if (result.hardcodedCount > 0) {
      console.log(
        `   硬编码文本 (${result.hardcodedCount}): ${result.componentCheck.hardcoded.slice(0, 2).join(', ')}${result.componentCheck.hardcoded.length > 2 ? '...' : ''}`
      );
    }
  ...
  console.log('='.repeat(80));
  console.log(
    `🎯 修复完成度: ${((perfectCount / translators.length) * 100).toFixed(1)}%`
  );
  ...
  if (perfectCount === translators.length) {
    ...
    console.log(
      `⚠️  还有 ${translators.length - perfectCount} 个翻译器需要进一步修复`
    );
  }
