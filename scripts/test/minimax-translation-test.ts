...
async function testTranslation(request: TranslationRequest) {
  ...
  if (request.systemInstruction) console.log(`🔧 系统指令: ${request.systemInstruction}`);
  ...
  try {
    ...
    if (response.ok && data.content) {
      ...
      console.log(`📊 Token使用: ${data.usage?.input_tokens} input, ${data.usage?.output_tokens} output`);
