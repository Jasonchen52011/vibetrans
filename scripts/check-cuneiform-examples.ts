...
function main() {
  try {
    const filePath = path.join(process.cwd(), 'messages/pages/cuneiform-translator/en.json');
    const content: CuneiformContent = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    ...
    console.log(`Average word count: ${(totalWords / cuneiformExamples.length).toFixed(1)}`);
  } catch (error) {
