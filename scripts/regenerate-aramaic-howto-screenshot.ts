...
async function regenerateAramaicHowToScreenshot() {
  console.log('开始重新生成 Aramaic-Translator-Pro How-To 截图...');
  ...
  try {
    ...
    if (result.filename && result.size > 0) {
      ...
      const jsonPath = path.join(process.cwd(), 'messages/pages/aramaic-translator/zh.json');
      const jsonContent = await fs.readFile(jsonPath, 'utf-8');
      const data = JSON.parse(jsonContent);

      // 更新 howto.image 为截图路径
      if (data.AramaicTranslatorPage.howto) {
        const imagePath = `/images/docs/${result.filename}`;
        data.AramaicTranslatorPage.howto.image = imagePath;
        console.log('更新 howto.image 路径为:', imagePath);
      }
...
// 执行重新生成
regenerateAramaicHowToScreenshot().then(() => {
